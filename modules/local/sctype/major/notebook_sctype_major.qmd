...
#| fig-height: 10
#| layout-ncol: 2
# SCP builds its own scale_*_manual internally and ggplot2 4.0 emits "No shared
# levels found between `names(values)` of the manual scale and the data's fill
# values" for it. Harmless, but without this it printed three times INTO the
# report, directly above the figure, where it reads as a broken plot.
#| warning: false
...
# `label = TRUE` stamped cluster numbers on every panel; at this panel size they
# overlapped into an unreadable smear and answered a question this figure is not
# asking. Same change as sctype/state.
fp <- FeaturePlot(
  object = seurat_object,
...
# The grid above compares all signatures at once; this shows one at a time at
# full size, which is the only way to actually read an individual score.
...
```{r violin_plot}
#| label: fig-violin
#| fig-cap: "Signature score distribution per cluster"
# Quarto scales a figure to the page width, so a WIDE figure shrinks everything
# in it: at 18in each panel was squashed to a sliver once fitted to the page.
# Narrower and taller renders each panel LARGER on screen. 7 panels, 2 columns,
# 4 rows -> ~5.5 x 5in each.
#| fig-width: 11
#| fig-height: 20
#| fig-align: center
#| warning: false
...
vp <- VlnPlot(
  object = seurat_object,
  features = names(non_malignant_markers),
  ncol = 2,                 # 2 wide instead of 3: each panel gets ~50% more width
  pt.size = 0               # 24k jittered points per panel hid the violins entirely
  )
