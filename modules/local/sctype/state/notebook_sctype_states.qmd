...
p <- SCP::CellDimPlot(
  ...
  # Legend on the right, not underneath. At the bottom the label block sat below
  # the axis and pushed the embedding into the top half of the figure; beside it,
  # the UMAP keeps its full height.
  theme(
...
```{r feature_plot_selector}
#| label: fig-feat-markers-selector
#| fig-cap: "Signature score on the UMAP — pick a signature from the dropdown on the right"

# One embedding, one score at a time, chosen from a dropdown. The grid below
# shows every signature at once for comparison; this is the readable view.
scratch_feature_selector(
  seurat_object,
...
# `label = TRUE` drew the cluster numbers on EVERY panel. At this panel size the
# labels from adjacent clusters overlapped into an unreadable smear, and they add
# nothing here -- the question this figure answers is "where is this signature
# expressed", not "which cluster is which". Panels also enlarged: the previous
# 10x12 inches split across ~16 panels gave each one about two inches.
fp <- FeaturePlot(
  object = seurat_object,
  features = names(non_malignant_markers),
  label = FALSE,
